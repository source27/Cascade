...
namespace Cascade.SourceGenerator
{
    ...
    {
        private const string UIAttributeName = "Cascade.Modules.UI.UIAttribute";
        private const string UIBaseName = "Cascade.Modules.UI.UIBase";
        private const string IUIArgsName = "Cascade.Modules.UI.IUIArgs`1";
        ...
        private static readonly DiagnosticDescriptor NotUIPage = new DiagnosticDescriptor(
            ...
            "Type '{0}' is marked with [UI] but does not inherit Cascade.Modules.UI.UIBase",
            "Cascade.UI",
        ...
        private static bool HasValidArgs(INamedTypeSymbol page)
        {
            ...
            foreach (var iface in args.AllInterfaces)
            {
                if (iface.OriginalDefinition == null ||
                    ...
                    iface.OriginalDefinition.ContainingNamespace?.ToDisplayString() != "Cascade.Modules.UI")
                    continue;
        ...
        private static string GenerateRegistry(IReadOnlyList<PageModel> pages)
        {
            ...
            builder.AppendLine("    {");
            builder.AppendLine("        public static void RegisterAll(global::Cascade.Modules.UI.UIRegistry registry)");
            builder.AppendLine("        {");

            foreach (var page in pages.OrderBy(item => item.Symbol.Name, StringComparer.Ordinal))
            {
                builder.AppendLine("            registry.Register(new global::Cascade.Modules.UI.UIPageRegistration(");
                ...
                builder.AppendLine("                (global::Cascade.Modules.UI.UILayer)" + page.Layer + ",");
                builder.AppendLine("                (global::Cascade.Modules.UI.UIPresentation)" + page.Presentation + ",");
                builder.AppendLine("                (global::Cascade.Modules.UI.UICachePolicy)" + page.Cache + ",");
                builder.AppendLine("                (global::Cascade.Modules.UI.UIInputPolicy)" + page.Input + ",");
                builder.AppendLine("                (global::Cascade.Modules.UI.UIBackPolicy)" + page.Back + ",");
                ...
                builder.AppendLine("                (global::Cascade.Modules.UI.UISafeAreaPolicy)" + page.SafeArea + ",");
            ...
            {
                builder.AppendLine();
                builder.AppendLine("        private sealed class " + page.Symbol.Name + "Factory : global::Cascade.Modules.UI.IUIPageFactory");
                builder.AppendLine("        {");
                builder.AppendLine("            public global::Cascade.Modules.UI.UIBase Create(GameObject root, object bindings, object pageContext)");
                builder.AppendLine("            {");
                ...
                builder.AppendLine("                page.Initialize(root, new " + page.BindingsTypeDisplay + "((global::Cascade.Modules.UI.UIBindingHost)bindings), pageContext);");
