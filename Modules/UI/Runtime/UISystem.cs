...
namespace Cascade.Modules.UI
{
    public sealed class UISystem : IUISystem
    {
        ...
        /// <summary>
        /// Loads the root prefab and builds the UI system. The caller owns the returned instance
        /// (typically stored on the game context); the registry is only used to resolve
        /// <see cref="IResourceService"/>, <see cref="IUpdateLoop"/> and <see cref="ILogService"/>.
        /// </summary>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var resources = services.Get<IResourceService>();
            services.TryGet<IUpdateLoop>(out var updateLoop);
            services.TryGet<ILogService>(out var log);
