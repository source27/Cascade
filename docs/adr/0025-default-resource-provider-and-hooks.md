...
- **默认组合**：日志 = `UnityLogService`；资源 = `UnityResourcesService`（`Resources/` 文件夹、原始字节用 TextAsset、场景进 Build Settings）；存档 = `PlayerPrefsSaveService`；音频 = `AudioService`；网络 = `NullNetworkService`。全部在主包，只用 Unity 自带 API。（图集精灵一项已改：见 [ADR 0026](0026-atlas-sprite-service-belongs-to-ui.md)——服务归 `Modules/UI`、不再默认注册。）
- **每个默认都由独立钩子产出**：`CreateLogService` / `CreateResourceService` / `CreateResourceInitOptions` / `CreateSaveService` / `CreateAudioService` / `CreateNetworkService` / `CreateUpdateLoop`。返回 null = 不注册该服务（`log` 与 `resource` 是流水线必需，返回 null 直接报错）。
