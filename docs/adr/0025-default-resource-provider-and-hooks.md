...
- **默认组合**：日志 = `UnityLogService`；资源 = `UnityResourcesService`（`Resources/` 文件夹、原始字节用 TextAsset、场景进 Build Settings）；存档 = `PlayerPrefsSaveService`；音频 = `AudioService`；网络 = `NullNetworkService`；图集 = `AtlasSpriteService`。全部在主包，只用 Unity 自带 API。
- **每个默认都由独立钩子产出**：`CreateLogService` / `CreateResourceService` / `CreateResourceInitOptions` / `CreateSaveService` / `CreateAudioService` / `CreateNetworkService` / `CreateAtlasSpriteService` / `CreateUpdateLoop`。返回 null = 不注册该服务（`log` 与 `resource` 是流水线必需，返回 null 直接报错）。
