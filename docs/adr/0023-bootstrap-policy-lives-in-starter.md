...
- `BootstrapEnvironment`（Dev/Beta/Gold）作为**词汇**留在主包；映射成策略（日志级别、玩家侧取值）由 Starter 子类写。
...
**Considered options：** 保留 slim `BootstrapConfiguration` 作值对象（否：字段与映射都下移后，容器只剩空壳）；把 `BootstrapEnvironment` 也下移（否：两 Starter 共享的纯词汇、无行为，留主包不构成策略）；把日志策反也留在主包用钩子只给级别（否：`CreateLogService` 与 `CreateResourceService` 对称，Starter 可整体换 logger）。
